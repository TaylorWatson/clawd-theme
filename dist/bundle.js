...
// dist/themes/normal.js
var normalTheme = {
  name: "normal",
  displayName: "Normal",
  description: "Classic orange Clawd",
  colors: {
    primary: "#D4A574",
    secondary: "#C49464"
  }
...
// dist/themes/valentines.js
var valentinesTheme = {
  name: "valentines",
  displayName: "Valentine's",
  description: "Pink Clawd with hearts",
  colors: {
    primary: "#FF69B4",
    secondary: "#FF1493",
    accent: "#FF69B4",
...
// dist/themes/summer.js
var summerTheme = {
  name: "summer",
  displayName: "Summer",
  description: "Sunny yellow and cyan Clawd",
  colors: {
    primary: "#FFD700",
    secondary: "#00CED1",
    accent: "#FFD700",
...
var CONFIG_DIR = path.join(process.env.HOME ?? "", ".claude");
...
function loadClawdConfig() {
  const settings = loadSettings();
  return settings.clawdTheme ?? { ...DEFAULT_CONFIG };
}
function saveClawdConfig(config) {
  const settings = loadSettings();
...
function colorize(text, hexColor) {
  const rgb = hexToRgb(hexColor);
  if (!rgb)
    return text;
  return source_default.rgb(rgb.r, rgb.g, rgb.b)(text);
}
function renderClawd(theme) {
...
function renderClawdWithMessage(theme, message) {
...
function renderAllThemes(themes2, currentTheme) {
  return themes2.map((theme) => {
...
`);
  ...
  console.log(renderAllThemes(themes2, config.theme));
  console.log("\nCommands:");
