...
export const themes: Record<ThemeName, ClawdTheme> = {
  normal: normalTheme,
  winter: winterTheme,
...
export function getTheme(name: ThemeName): ClawdTheme {
  return themes[name] ?? normalTheme;
}

export function getSeasonalTheme(): ClawdTheme {
  const now = new Date();
...
export function getAllThemes(): ClawdTheme[] {
